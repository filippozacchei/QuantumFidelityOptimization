...
#     # only used for the 2D visualisation panel
#     nx, ny = 10, 10
#     xs = range(lb[1], ub[1], length=nx)
#     ys = d ≥ 2 ? range(lb[2], ub[2], length=ny) : range(0.0, 1.0, length=ny)

#     # cache true objective on grid (for visual error map)
#     Ztrue = d == 2 ? [f_eval([x,y]) for y in ys, x in xs] : zeros(ny, nx)
...
#         cand = [x for x in cand if is_new_point(x, X; tol=0.0)]
#         if isempty(cand)
#             # FIX: produce a single candidate in the correct dimension d
#             cand = [[rand()*(ub[j]-lb[j]) + lb[j] for j in 1:d]]
#         end
